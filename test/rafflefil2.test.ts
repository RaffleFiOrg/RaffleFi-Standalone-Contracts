...
import { createMerkleTree, WhitelistData } from "./utils";
...
describe("RaffleFi", function () {
    ...
    beforeEach(async () => {
        ...
        const mockRandomizerFactory = await ethers.getContractFactory("MockRandomizer")
        mockRandomizer = await mockRandomizerFactory.deploy()

        raffleFiFactory = await ethers.getContractFactory("RaffleFiL2")
        raffleFi = await raffleFiFactory.deploy(
            WETH.address,
            mockRandomizer.address,
            100000,
            1
        )

        const mockRaffleFiVRFFactory = await ethers.getContractFactory("MockRaffleSetterL2")
        ...
        mockRaffleFiSetter = await mockRaffleFiVRFFactory.connect(user1).deploy(
            WETH.address,
            mockRandomizer.address,
            100000,
            1
        ...
        const BadReceiverFactory = await ethers.getContractFactory("MockBadRaffleCreatorL2")
        badReceiver = await BadReceiverFactory.deploy(raffleFi.address)
    ...
    describe("Setup", async () => {
        it("should have set the correct state variables", async () => {
            ...
            expect(await raffleFi.requestConfirmations()).to.equal(1)
        })
    })
    ...
    describe("Cancel", () => {
        ...
        it("should not allow to cancel a raffle after the winner was chosen", async () => {
            ...
            // complete raffle
            const completeFee = await raffleFi.estimateVRFFee()
            const tx = await mockRaffleFiVRF.connect(user1).completeRaffle(1, true, {value: completeFee})
            const receipt = await tx.wait()
            const requestId = receipt.events[1].args[0].toString()

            // send random number
            await mockRandomizer.sendRandomNumber(mockRaffleFiVRF.address) 
    ...
    describe.only("Complete", () => {
        ...
        it("should decrease the Ether balance of the owner", async () => { 
        ...
        it("should successfully store a VRF request id", async () => {
            const completeFee = await raffleFi.estimateVRFFee()
            const tx = await raffleFi.connect(user1).completeRaffle(1, true, {value: completeFee})
            const receipt = await tx.wait()
            const requestId = receipt.events[1].args[0].toString()
        ...
        it("should not allow to complete someone else's raffle", async () => {
            const completeFee = await raffleFi.estimateVRFFee()
            await expect(raffleFi.connect(user2).completeRaffle(1, true, {value: completeFee}))
        ...
        it("should set the raffle to REFUNDED if the creator does not agree to complete with less tickets sold than expected", async () => {
            ...
            await network.provider.send("evm_mine")
            const completeFee = await raffleFi.estimateVRFFee()
            await raffleFi.connect(user1).completeRaffle(2, false, {value: completeFee})
            const raffle = await raffleFi.getRaffleDetails(2)
            expect(raffle.raffleState).to.be.eq(3) // REFUNDED
        })
        it("should send the NFT back to the raffle creator if the raffle doet not complete (erc721 raffle)", async () => {
            ...
            const completeFee = await raffleFi.estimateVRFFee()
            await raffleFi.connect(user1).completeRaffle(3, false, {value: completeFee}) 
        ...
        it("(bad receiver reverting on receive) should refund the ether as WETH for a erc20 raffle with ether as currency and not enough tickets sold + creator not agreeing to complete", async () => {
            ...
            await network.provider.send("evm_mine")
            const completeFee = await raffleFi.estimateVRFFee()
            await badReceiver.connect(user1).completeRaffle(3, false, {value: completeFee})
            const wethBalance = await WETH.balanceOf(badReceiver.address)
            expect(wethBalance).to.be.eq(utils.parseUnits("1", 18))
            const raffle = await raffleFi.getRaffleDetails(3)
            expect(raffle.raffleState).to.be.eq(3) // REFUNDED
        ...
        it("should not allow to complete an ERC721 raffle if not all tickets were sold or deadline passed", async () => {
            ...
            const completeFee = await raffleFi.estimateVRFFee()
            await expect(raffleFi.connect(user1).completeRaffle(3, true, {value: completeFee}))
        ...
        it("should not allow to complete an ERC20 raffle if not all tickets were sold or deadline passed", async () => {
            const completeFee = await raffleFi.estimateVRFFee()
            await expect(raffleFi.connect(user1).completeRaffle(2, true, {value: completeFee}))
            .to.be.revertedWithCustomError(raffleFi, "RaffleNotEnded")
        })
        it("should emit an event when completing a raffle", async () => {
            const completeFee = await raffleFi.estimateVRFFee()
            expect(await raffleFi.connect(user1).completeRaffle(1, true, {value: completeFee}))
        ...
        it("should not complete a raffle that is cancelled", async () => {
            const completeFee = await raffleFi.estimateVRFFee()
            await raffleFi.connect(user1).cancelRaffle(1)
            await expect(raffleFi.connect(user1).completeRaffle(1, true, {value: completeFee}))
    ...
    describe("Claim", () => {
        ...
        it("should prevent cancelling after claiming", async () => {
            ...
            .to.be.revertedWithCustomError(raffleFi, "RaffleCannotBeCancelled")
        })
        it("should prevent buying a ticket after a raffle was claimed (still within the raffle deadline)", async () => {
            ...
            .to.be.revertedWithCustomError(raffleFi, "RaffleEnded")
        })
    ...
    describe("fulfilRandomWords", () => {
        ...
        beforeEach(async () => {
            ...
            // complete raffle
            const completeFee = await mockRaffleFiVRF.estimateVRFFee()
            const tx = await mockRaffleFiVRF.connect(user1).completeRaffle(1, true, {value: completeFee})
            const receipt = await tx.wait()
            requestId = receipt.events[1].args[0].toString()
        })
        it("should fail when the raffle is cancelled", async () => {
            await mockRaffleFiVRF.connect(user1).cancelRaffle(1)
            await expect(mockRandomizer.sendRandomNumber(mockRaffleFiVRF.address))
            .to.be.revertedWithCustomError(raffleFi, "RaffleWasCancelled")
        })
        it("should emit an event", async () => {
            // send random number
            expect(mockRandomizer.sendRandomNumber(mockRaffleFiVRF.address)) // 131 % 10 
            .to.emit(raffleFi, "RaffleStateChanged")
            .withArgs(1, 2) // FINISHED -> COMPLETED
        })
        it("should draw the correct winner", async () => {
            // send random number
            await mockRandomizer.sendRandomNumber(mockRaffleFiVRF.address) // 15 % 10 = 5
